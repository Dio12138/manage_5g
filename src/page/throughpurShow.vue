<template>
    <div style="width: 100%;height: 100%">
        ...
        <div v-show="true" style="width: 100%;height: 100%;display: flex">
            ...
            <img class="img2" src="../assets/img/throughput_dl.png">
            <div style="width: 40%;height:70%;margin:0" id="container1"></div>
            <img class="img2" src="../assets/img/throughtput_ul.png">
        </div>
...
<script>
	...
	export default {
		...
		methods: {
			showMap() {
				this.$api.get('DY_DATE/getMovingThroughputById', {
					...
					if (response.status >= 200 && response.status < 300) {
						...
						if (response.data.success1) {
							if(response.data.success2){
								...
								this.throughput_ul = response.data.throughput_ul;
								this.throughput_dl = response.data.throughput_dl;
			...
			showMap1(){
				...
				for (var i = 0; i <tempArr.length-1; i++) {
					var midThroughputDl=(this.throughput_dl[i]+this.throughput_dl[i+1])/2;
					var polyline = new BMap.Polyline([
							tempArr[i],
							tempArr[i+1],
						],
						{strokeColor:this.getColor1(midThroughputDl), strokeWeight:6, strokeOpacity:0.5}
					);
			...
			showMap2(){
				...
				for (var i = 0; i <tempArr.length-1; i++) {
					var midThroughputUl=(this.throughput_ul[i]+this.throughput_ul[i+1])/2;
					var polyline = new BMap.Polyline([
							tempArr[i],
							tempArr[i+1],
						],
						{strokeColor:this.getColor2(midThroughputUl), strokeWeight:6, strokeOpacity:0.5}
					);
					map.addOverlay(polyline);
				}
			...
			getColor1(mid){
				var colorVal=[1000,600,400,200,10,0];
				var myColor=["#001900","#00fa00","#28262c","#ffff00","#f3e739","#fd0000"];
