<template>
    <div style="width: 100%;height: 100%">
        ...
        <div v-show="true" style="width: 100%;height: 100%;display: flex">
            ...
            <img class="img1" src="../assets/img/sinrcsi.png">
            <div style="width: 40%;height:70%;margin:0" id="container1"></div>
            <img class="img1" src="../assets/img/avgsinr.png">
        </div>
...
<script>
	...
	export default {
		...
		methods: {
			showMap() {
				this.$api.get('DY_DATE/getMovingSinrById', {
					...
					if (response.status >= 200 && response.status < 300) {
						...
						if (response.data.success1) {
							if(response.data.success2){
								...
								this.sinr_ul = response.data.sinr_ul;
								this.sinr_dl = response.data.sinr_dl;
			...
			showMap1(){
				...
				for (var i = 0; i <tempArr.length-1; i++) {
					var midSinrDl=(this.sinr_dl[i]+this.sinr_dl[i+1])/2;
					var polyline = new BMap.Polyline([
							tempArr[i],
							tempArr[i+1],
						],
						{strokeColor:this.getColor(midSinrDl), strokeWeight:6, strokeOpacity:0.5}
					);
			...
			showMap2(){
				...
				for (var i = 0; i <tempArr.length-1; i++) {
					var midSinrUl=(this.sinr_ul[i]+this.sinr_ul[i+1])/2;
					var polyline = new BMap.Polyline([
							tempArr[i],
							tempArr[i+1],
						],
						{strokeColor:this.getColor(midSinrUl), strokeWeight:6, strokeOpacity:0.5}
					);
					map.addOverlay(polyline);
				}
			...
			getColor(mid){
				var colorVal=[3,10,16,25,50];
				var myColor=["#fe0000","#edeb25","#f6ff00","#00f200","#002500"];
